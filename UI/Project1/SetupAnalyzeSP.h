...
namespace Project1
{
	...
	private:
		...
		int SaveSettings(void)
		{
			...
			(*Setting)[group]["FunctionDelayCycle"] = std::to_string(std::stod(TKUTIL::SystemStringToString(textBox6->Text).c_str()));
			(*Setting)[group]["FunctionSweepCycle"] = std::to_string(std::stod(TKUTIL::SystemStringToString(textBox7->Text).c_str()));
			(*Setting)[group]["TimeRegionStart"] = std::to_string(std::stod(TKUTIL::SystemStringToString(textBox14->Text).c_str()));
			(*Setting)[group]["TimeRegionStop"] = std::to_string(std::stod(TKUTIL::SystemStringToString(textBox13->Text).c_str()));
		...
		int LoadSettings(void)
		{
			...
			textBox14->Text = gcnew System::String((std::to_string(std::stod(
				((*Setting)[group]["TimeRegionStart"]).c_str()))).c_str());
			textBox13->Text = gcnew System::String((std::to_string(std::stod(
				((*Setting)[group]["TimeRegionStop"]).c_str()))).c_str());
